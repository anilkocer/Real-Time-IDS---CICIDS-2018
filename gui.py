...
def process_packet(packet, listbox):
    global clf, scaler, le  # clf, scaler ve le değişkenlerini global olarak işaret ediyoruz
    ...
    if IP in packet:
        ...
        print(f"Benign Probability: {prediction_proba[0][0]*100}")
        print(f"Attack Probability: {prediction_proba[0][1]*100}")
        
        # Çıktıları arayüzde listbox'a ekleme
        listbox.insert(tk.END, f'Dst Port: {feature["Dst Port"]}, Protocol: {feature["Protocol"]}, Flow Duration: {feature["Flow Duration"]}, Tot Fwd Pkts: {feature["Tot Fwd Pkts"]}, Prediction: {prediction[0]}, Benign Probability: {prediction_proba[0][0]*100}, Attack Probability: {prediction_proba[0][1]*100}')
        listbox.yview(tk.END)  # Listbox'ı otomatik olarak en alta kaydır

# Ağ trafiğini dinleme fonksiyonu
def start_sniffing(listbox):
    sniff(prn=lambda packet: process_packet(packet, listbox))
...
def create_gui():
    ...
    start_button = tk.Button(root, text="Start", command=lambda: threading.Thread(target=start_sniffing, args=(listbox,)).start())
    start_button.pack()

    listbox = tk.Listbox(root, width=150, height=30)
    listbox.pack()
